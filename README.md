...
- The same base class also exposes a reusable WireMock container for stubbing third-party REST calls; by default it returns a `READY` status for `/orders/{id}/status` so flows that rely on the third-party call (like publishing to SNS) work without per-test setup. The `ThirdPartyApiIntegrationTest` class demonstrates registering a custom stub via the container's mapped port and asserting the client call against the stubbed endpoint.
